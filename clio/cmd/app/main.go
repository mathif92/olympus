...
func main() {
	var (
		addr = flag.String("addr", getenv("CLIO_ADDR", ":8087"), "listen address")
	)
	flag.Parse()

	dbCfg := database.Config{
		PostgresURL: getenv("POSTGRES_DSN",
			"host=localhost port=15436 user=olympus password=olympus_secret dbname=olympus_databases sslmode=disable"),
	...
	// Select the pluggable data-plane provisioner (mock by default). The real
	// backend launches actual PostgreSQL containers via a Docker daemon.
	var provisioner pkg.Provisioner
	switch getenv("PROVISIONER", "mock") {
	case "mock":
		provisioner = pkg.NewMockProvisioner()
	case "docker":
		provisioner = pkg.NewDockerProvisioner(getenv("CLIO_POSTGRES_IMAGE", "postgres:16-alpine"))
	default:
		log.Fatalf("unknown PROVISIONER %q (want mock|docker)", getenv("PROVISIONER", "mock"))
	}

	clio := pkg.NewClio(dbClient, provisioner)
	ch := handler.NewClioHandler(clio)
	...
	mux.Handle("/", authzClient.Middleware(authz.ServiceMapper("clio"))(ch.Router()))

	log.Printf("📚 Clio running on %s (provisioner: %T)...", *addr, provisioner)

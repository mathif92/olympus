...
// isHexID reports whether s looks like a 32-char hex identifier (the format
// generated by newID), used to disambiguate project IDs from project names.
...
// resolveProjectByNameOrID resolves a project by name, or by its ID when the
// value looks like a 32-hex identifier (e.g. the project claim from a token).
// The lookup is always scoped to the owning account.
func (s *ThemisStore) resolveProjectByNameOrID(ctx context.Context, accountID, nameOrID string) (*database.Project, error) {
	if isHexID(nameOrID) {
		var p database.Project
		err := s.DB.QueryRow(ctx, `
			SELECT id, account_id, name, COALESCE(description,''), resource_count, created_at, status
	...
	return s.resolveProject(ctx, accountID, nameOrID)
}
...
func (s *ThemisStore) Authorize(ctx context.Context, accountID, projectName, principalType, principalName, action, resource string) (*EvaluationDecision, error) {
	...
	proj, err := s.resolveProjectByNameOrID(ctx, accountID, projectName)
	if err != nil {
		return nil, err
	}
	principalID, err := s.resolvePrincipal(ctx, proj.ID, principalType, principalName)

...
func main() {
	var (
		addr = flag.String("addr", getenv("MNEME_ADDR", ":8088"), "listen address")
	)
	flag.Parse()

	dbCfg := database.Config{
		PostgresURL: getenv("POSTGRES_DSN",
			"host=localhost port=15437 user=olympus password=olympus_secret dbname=olympus_caches sslmode=disable"),
	...
	// backend launches actual Redis containers via a Docker daemon.
	var provisioner pkg.Provisioner
	switch getenv("PROVISIONER", "mock") {
	case "mock":
		provisioner = pkg.NewMockProvisioner()
	case "docker":
		provisioner = pkg.NewDockerProvisioner(getenv("MNEME_REDIS_IMAGE", "redis:7-alpine"))
	default:
		log.Fatalf("unknown PROVISIONER %q (want mock|docker)", getenv("PROVISIONER", "mock"))
	}

	mneme := pkg.NewMneme(dbClient, provisioner)
	ch := handler.NewMnemeHandler(mneme)
	...
	mux.Handle("/", authzClient.Middleware(authz.ServiceMapper("mneme"))(ch.Router()))

	log.Printf("🧠 Mneme running on %s (provisioner: %T)...", *addr, provisioner)
